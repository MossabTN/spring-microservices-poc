...
public class TokenRelayRequestInterceptor implements RequestInterceptor {
    ...
    public void apply(RequestTemplate template) {
        if(template.headers().get(AUTHORIZATION) == null){
            Optional<String> authorizationHeader = authorizationHeaderUtil.getSystemAuthorizationHeader();
            authorizationHeader.ifPresent(s -> template.header(AUTHORIZATION, "Bearer "+s));
        }
    }
}
